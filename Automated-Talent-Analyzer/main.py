...
st.title("Automated Talent Analyzer 📝")
...
if st.session_state.form_submitted:
    ...
    st.markdown("---")
    st.header("💡 Personalized Interview Practice Tips")
    st.subheader("Based on your ATS review score, focus on these core techniques:")

    with st.expander("⭐ 1. Structure Your Answers with the STAR Method"):
        st.write("""
        When answering behavioral questions, break your response down into:
        - **S**ituation: Describe the context or challenge.
        - **T**ask: What was your specific responsibility?
        - **A**ction: Explain the exact steps *you* took to solve it.
        - **R**esult: Highlight the positive outcome with metrics (e.g., 'improved speed by 20%').
        """)

    with st.expander("🛡️ 2. Handle Critical Technical Gaps"):
        st.write("""
        If the report flags missing skills from the job description:
        - Never say 'I don't know' during the interview.
        - Instead, say: *'While I haven't implemented [X skill] in production yet, I am highly familiar with its concepts through my work with [similar skill Y] and can ramp up quickly.'*
        """)

    with st.expander("❓ 3. Top Questions to Ask the Interviewer"):
        st.write("""
        Always ask smart questions at the end of the interview to show deep interest:
        - *'What does success look like in this role during the first 90 days?'*
        - *'What is the biggest technical bottleneck the engineering team is currently trying to solve?'*
        """)
